...
class ProjectState extends State<Project> {
  ...
  addProject(title: string, description: string, numOfPeople: number) {
    ...
    for (const listenerFn of this.listeners) {
      listenerFn(this.projects.slice()); //slice to return only a copy not the original to deal with pass by reference for objects
    }
  }
}
...
interface Validatable {
  ...
  maxLength?: number;
  min?: number;
  max?: number; //? means it can be undefined. this is how you amke it optional
}
  ...
  implements Draggable
{
  ...
  dragStartHandler(event: DragEvent) {
    ...
    event.dataTransfer!.effectAllowed = "move"; //changes cursor and informs browser
  }
  @autobind
  ...
  implements DragTarget
{
  ...
  dragOverHandler(event: DragEvent) {
    if (event.dataTransfer && event.dataTransfer.types[0] === "text/plain") {
      event.preventDefault(); //default for JS D&D is to not allow drop
  ...
  private renderProjects() {
    ...
    //empty the list out to prevent duplicates - simple
    listEl.innerHTML = "";
